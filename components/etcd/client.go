...
func (c *Client) WatchKey(key string, callback func(any)) error {
	...
	go func() {
		defer func() {
			// goroutine 退出（ctx 取消或 etcd 关闭）时，自动从 map 清理本 watcher 项，
			// 避免下次 WatchKey 同名 key 时被 "already exists" 错误拒绝。
			c.mu.Lock()
			if existing, ok := c.watchers[key]; ok {
				// 仅在仍是本 cancel 时清理，防止与并发的 StopWatch+WatchKey 冲突
				_ = existing
