...
var identifierExpr = regexp.MustCompile(`^[A-Za-z_][A-Za-z0-9_.]*$`)

func conditionMatches(expr string, vars map[string]any) bool {
	matched, err := evalCondition(expr, vars)
...
func evalCondition(expr string, vars map[string]any) (bool, error) {
	...
	right, ok := vars[rightLiteral]
	if !ok || !identifierExpr.MatchString(rightLiteral) {
		var err error
