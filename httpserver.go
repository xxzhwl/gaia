...
func (h *HttpServer) Shutdown(ctx context.Context) error {
	...
	h.sendLifecycleNotify(protocol, "停止")
...
func (h *HttpServer) https() error {
	...
	h.sendLifecycleNotify("HTTPS", "启动")
...
// sendLifecycleNotify 发送HTTP服务生命周期通知
func (h *HttpServer) sendLifecycleNotify(protocol, action string) {
	defer CatchPanic()
	if err := SendLifecycleNotify(LifecycleNotifyInfo{
		Kind:     "service",
		Name:     protocol,
		Action:   action,
		...
		Fields: map[string]any{
			"idle_timeout":     h.server.IdleTimeout.String(),
			"read_timeout":     h.server.ReadTimeout.String(),
			"shutdown_timeout": h.shutdownTimeout.String(),
			"tls_enabled":      len(h.certFile) > 0 && len(h.keyFile) > 0,
			"write_timeout":    h.server.WriteTimeout.String(),
		},
	}); err != nil {
		WarnF("[%s] 发送%s通知失败: %s", protocol, action, err.Error())
	}
}
