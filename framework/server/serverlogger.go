...
// serverApiLogger 包级共享的日志实例，避免每次请求创建新 logger
var serverApiLogger *logImpl.DefaultLogger

func init() {
	serverApiLogger = logImpl.NewDefaultLogger().SetTitle("ServerApi")
}
		...
		defer func() {
			...
			if gaia.GetSafeConfBool(s.schema+".Logger.EnablePushLog") && !ctx.GetBool(DisabledPushLoggerKey) {
				...
				ctx.Request.Header.VisitAll(func(key, value []byte) {
					...
					// Authorization header 脱敏：仅记录前20字符
					if keyStr == "Authorization" && len(valueStr) > 20 {
						valueStr = valueStr[:20] + "..."
					}
					reqHeader[keyStr] = []string{valueStr}
				})
