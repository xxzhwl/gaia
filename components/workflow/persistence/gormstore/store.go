...
func (r *definitionRepo) Update(ctx context.Context, def domain.ProcessDefinition) error {
	...
	return r.db.WithContext(ctx).Model(&ProcessDefinitionModel{}).
		Where("id = ?", def.ID).
...
func (r *instanceRepo) SaveExecution(ctx context.Context, execution domain.Execution) error {
	return r.db.WithContext(ctx).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{
			"instance_id", "parent_id", "node_id", "status", "join_key", "created_at", "updated_at",
		}),
	}).Create(executionToModel(execution)).Error
}
...
func (r *instanceRepo) SaveActivity(ctx context.Context, activity domain.ActivityInstance) error {
	return r.db.WithContext(ctx).Clauses(clause.OnConflict{
		Columns: []clause.Column{{Name: "id"}},
		DoUpdates: clause.AssignmentColumns([]string{
			"instance_id", "execution_id", "node_id", "node_type", "node_name", "status", "start_time", "end_time", "retry_count", "error_message",
		}),
	}).Create(activityToModel(activity)).Error
}
...
func (r *outboxRepo) List(ctx context.Context) ([]domain.OutboxEvent, error) {
	var models []OutboxEventModel
