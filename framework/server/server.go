...
type Server struct {
	*server.Hertz
	...
	stopCleanupOnce   sync.Once // 防止重复关闭
...
func (s *Server) rateLimitPlugin() app.HandlerFunc {
	// 初始化时缓存限流配置
	s.rateLimitCapacity = gaia.GetSafeConfInt64WithDefault(s.schema+".RateLimit.Capacity", 100)
	if s.rateLimitCapacity <= 0 {
		s.rateLimitCapacity = 100
	}
	s.rateLimitRate = gaia.GetSafeConfFloat64WithDefault(s.schema+".RateLimit.Rate", 50.0)
	if s.rateLimitRate <= 0 {
		s.rateLimitRate = 50.0
	}

	return func(ctx context.Context, c *app.RequestContext) {
		capacity := int(s.rateLimitCapacity)
		rate := s.rateLimitRate
		...
		// 获取或创建限流器
		rl := s.getRateLimiter(key, capacity, rate)
