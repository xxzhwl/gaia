...
func (r FeiShuRobot) SendNotify(title string, content string) error {
	return r.SendRichText(title, []Content{{Text: content, Tag: "text"}})
}
...
func (r FeiShuRobot) request(msgType string, content map[string]any) error {
	...
	respBody, _, err := httpclient.NewHttpRequest(r.Hook).WithTitle("FeiShuRobot").Post(msg)
	if err != nil {
		return err
	}
	var resp RobotResp
	if jsonErr := json.Unmarshal(respBody, &resp); jsonErr == nil && resp.Code != 0 {
		return fmt.Errorf("飞书机器人发送失败: code=%d, msg=%s", resp.Code, resp.Msg)
	}
	return nil
}
