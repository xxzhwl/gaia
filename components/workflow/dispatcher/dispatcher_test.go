...
func TestDispatcherSendsExternalTaskAndMarksOutboxSent(t *testing.T) {
	...
	d := New(rt, WithCallbackBaseURL("https://workflow.example.com"), WithEndpointValidator(&DefaultEndpointGuard{AllowPrivate: true}))
...
func TestDispatcherMarksOutboxFailedForNon2xxResponse(t *testing.T) {
	...
	d := New(rt,
		...
		WithAlertHandler(AlertHandlerFunc(func(_ context.Context, alert Alert) error {
			alerts = append(alerts, alert)
			return nil
		})),
		WithEndpointValidator(&DefaultEndpointGuard{AllowPrivate: true}),
	)
	...
	if len(alerts) != 1 || alerts[0].Status != domain.OutboxStatusFailed || alerts[0].EventID != dispatchEvent.ID {
		t.Fatalf("expected failed alert for dispatch outbox, got %#v", alerts)
	}
}
...
func TestDispatcherResolvesAutomationTaskReference(t *testing.T) {
	...
	d := New(rt, WithAutomationRegistry(registry), WithEndpointValidator(&DefaultEndpointGuard{AllowPrivate: true}))
...
func TestDispatcherMarksOutboxDeadAfterMaxAttempts(t *testing.T) {
	...
	if len(alerts) != 1 || alerts[0].Status != domain.OutboxStatusDead || alerts[0].EventID != dispatchEvent.ID {
		t.Fatalf("expected dead alert for dispatch outbox, got %#v", alerts)
	}
	tasks := rt.Tasks(dispatchEvent.Payload["processInstanceId"].(string))
	if tasks[1].Status != domain.TaskStatusFailed {
		t.Fatalf("expected external task failed after dead outbox, got %#v", tasks[1])
	}
	instance, ok := rt.GetInstance(tasks[1].InstanceID)
	if !ok {
		t.Fatalf("get instance %s", tasks[1].InstanceID)
	}
	if instance.Status != domain.InstanceStatusFailed {
		t.Fatalf("expected instance failed after dead outbox, got %#v", instance)
	}
}
...
func TestDispatcherRunStopsWhenContextIsCanceled(t *testing.T) {
	...
	rt := startApprovedFlowWaitingExternalTask(t, bizServer.URL)
	d := New(rt, WithPollInterval(time.Millisecond), WithBatchSize(1), WithEndpointValidator(&DefaultEndpointGuard{AllowPrivate: true}))
