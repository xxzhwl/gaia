...
func (l *LocalDbLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	if err != nil && (!errors.Is(err, gorm.ErrRecordNotFound)) {
		...
		if l.Config.LogLevel >= logger.Error {
			...
			if rows == -1 {
				logContent = fmt.Sprintf(traceLogFmt, utils.FileWithLineNum(), err, sqlType, mainTable, float64(elapsed.Nanoseconds())/1e6, "-", sql)
			} else {
				logContent = fmt.Sprintf(traceLogFmt, utils.FileWithLineNum(), err, sqlType, mainTable, float64(elapsed.Nanoseconds())/1e6, rows, sql)
			}
		}
		...
		if l.Config.LogLevel >= logger.Warn {
			...
			if rows == -1 {
				logContent = fmt.Sprintf(traceLogFmt, utils.FileWithLineNum(), slowLog, sqlType, mainTable, float64(elapsed.Nanoseconds())/1e6, "-", sql)
			} else {
				logContent = fmt.Sprintf(traceLogFmt, utils.FileWithLineNum(), slowLog, sqlType, mainTable, float64(elapsed.Nanoseconds())/1e6, rows, sql)
			}
		}
	} else {
		// 正常SQL日志
		if l.Config.LogLevel >= logger.Info {
			needLog = true
			logLevel = gaia.LogInfoLevel
			logContent = fmt.Sprintf(traceLogFmt, utils.FileWithLineNum(), "", sqlType, mainTable, float64(elapsed.Nanoseconds())/1e6, rows, sql)
		}
	}
