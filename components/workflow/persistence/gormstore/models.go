...
type ProcessDefinitionModel struct {
	...
	EditVersion int `gorm:"not null;default:0"`
}
...
// AsyncTaskBindingModel 映射 workflow_async_task_bindings 表。
type AsyncTaskBindingModel struct {
	ID                  uint64    `gorm:"column:id;primaryKey;autoIncrement"`
	AsyncTaskID         int64     `gorm:"column:async_task_id;uniqueIndex;not null"`
	WorkflowTaskID      string    `gorm:"column:workflow_task_id;index;size:64;not null"`
	ProcessInstanceID   string    `gorm:"column:process_instance_id;index;size:64;not null"`
	DefinitionID        string    `gorm:"column:definition_id;size:64"`
	DefinitionKey       string    `gorm:"column:definition_key;size:128"`
	DefinitionName      string    `gorm:"column:definition_name;size:255"`
	DefinitionVersion   int       `gorm:"column:definition_version"`
	InstanceName        string    `gorm:"column:instance_name;size:255"`
	NodeID              string    `gorm:"column:node_id;size:128"`
	NodeName            string    `gorm:"column:node_name;size:255"`
	AutomationTaskKey   string    `gorm:"column:automation_task_key;index;size:128;not null"`
	Theme               string    `gorm:"column:theme;index;size:64;not null"`
	ServiceName         string    `gorm:"column:service_name;size:128;not null"`
	MethodName          string    `gorm:"column:method_name;size:128;not null"`
	CallbackToken       string    `gorm:"column:callback_token;size:255"`
	CompleteCallbackURL string    `gorm:"column:complete_callback_url;type:text"`
	FailCallbackURL     string    `gorm:"column:fail_callback_url;type:text"`
	Status              string    `gorm:"column:status;index;size:32;not null"`
	CallbackStatus      string    `gorm:"column:callback_status;index;size:32;not null"`
	LastError           string    `gorm:"column:last_error;type:text"`
	CreatedAt           time.Time `gorm:"column:created_at;not null"`
	UpdatedAt           time.Time `gorm:"column:updated_at;not null"`
}

// TableName 返回 workflow 与 asynctask 的绑定表名。
func (AsyncTaskBindingModel) TableName() string {
	return "workflow_async_task_bindings"
}
