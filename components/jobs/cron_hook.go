...
func (r *RunJob) updateCronHookJobs(jobs []job) error {
	...
	for _, job := range needAddJobs {
		if err := r.addCronHookJob(job); err != nil {
			...
			gaia.SendSystemAlarm("AddCronHookJob", err.Error())
			continue
		}
	}
...
func (r *RunJob) DoCronHookJob(job cronJob) {
	...
	go func() {
		defer func() {
			if rr := recover(); rr != nil {
				...
				gaia.SendSystemAlarm(fmt.Sprintf("JobHookPanic[%s]", job.JobName),
	...
	case <-ctx.Done():
		...
		gaia.SendSystemAlarm(fmt.Sprintf("JobHookTimeout[%s]", job.JobName),
