...
func (r *RunJob) updateCronHookJobs(jobs []job) error {
	...
	needAddJobsId, needDeleteJobsId := gaia.DifferenceList(jobsId, r.currentHookJobIds)
	interSectJobsId := gaia.IntersectList(jobsId, r.currentHookJobIds)
	...
	for i, _ := range r.currentCronHookJobMap {
...
func (r *RunJob) DoCronHookJob(job cronJob) {
	...
	resInfo, _, errTemp := httpclient.NewHttpRequest(job.HookUrl).WithMethod(http.MethodGet).WithBody(job.Args).Do()
	if errTemp != nil {
