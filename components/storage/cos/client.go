...
func (c *Client) MoveObject(ctx context.Context, srcObject, destObject string) error {
	...
	if err := c.DeleteObject(ctx, srcObject); err != nil {
		// 如果删除原对象失败，回滚：尝试删除新复制的对象
		if cleanErr := c.DeleteObject(ctx, destObject); cleanErr != nil {
			return errors.Wrapf(err,
				"移动对象失败：删除原对象出错；回滚删除新对象同样失败(%v)，可能存在残留: %s",
				cleanErr, destObject)
		}
...
// getBucketName 获取完整的Bucket名称（内部委托给 Config.getBucketName 以保证一致）
func (c *Client) getBucketName() string {
	return c.config.getBucketName()
}

// getBucketName 获取完整的Bucket名称。
// COS 规范 Bucket 格式为 `<name>-<AppID>`（如 mybucket-1250000000）。
// 若配置中 Bucket 未包含 -AppID 后缀，此处自动拼接。
func (c *Config) getBucketName() string {
	...
	suffix := "-" + c.AppID
	if strings.HasSuffix(c.Bucket, suffix) {
		return c.Bucket
	}
	return c.Bucket + suffix
}
