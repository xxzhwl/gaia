...
func (r *PersistentRuntime) UpdateDefinition(ctx context.Context, definitionID string, def domain.ProcessDefinition) (domain.ProcessDefinition, error) {
	...
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		...
		if def.EditVersion != existing.EditVersion {
			return fmt.Errorf("definition %s edit conflict: expected version %d got %d",
				definitionID, def.EditVersion, existing.EditVersion)
		}
...
// SetDefinitionStatus 停用（DISABLED）或重新启用（DEPLOYED）流程定义。
func (r *PersistentRuntime) SetDefinitionStatus(ctx context.Context, definitionID string, status domain.DefinitionStatus) (domain.ProcessDefinition, error) {
	var result domain.ProcessDefinition
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		def, err := repos.Definitions().Get(ctx, definitionID)
		if err != nil {
			return err
		}
		if err := validateDefinitionStatusTransition(def.Status, status); err != nil {
			return err
		}
		if err := repos.Definitions().SetStatus(ctx, definitionID, status); err != nil {
			return err
		}
		def.Status = status
		result = def
...
func (r *PersistentRuntime) CompleteTask(ctx context.Context, req CompleteTaskRequest) (domain.ProcessInstance, error) {
	...
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		var err error
		result, workflowErr, err = r.completeTaskInTx(ctx, repos, req, true)
		return err
	})
	if err != nil {
		return result, err
...
func (r *PersistentRuntime) completeTaskInTx(ctx context.Context, repos persistence.Repositories, req CompleteTaskRequest, verifyToken bool) (domain.ProcessInstance, error, error) {
	task, err := repos.Tasks().Get(ctx, req.TaskID)
	if err != nil {
		return domain.ProcessInstance{}, nil, err
	}
	if verifyToken {
		if err := verifyCallbackToken(task, req.CallbackToken); err != nil {
			return domain.ProcessInstance{}, nil, err
		}
	}
	...
	if task.Status != domain.TaskStatusWaiting && task.Status != domain.TaskStatusDispatched && task.Status != domain.TaskStatusCreated && task.Status != domain.TaskStatusClaimed {
		return domain.ProcessInstance{}, nil, fmt.Errorf("task %s status %s cannot be completed", task.ID, task.Status)
	}
	instance, err := repos.Instances().Get(ctx, task.InstanceID)
	if err != nil {
		return domain.ProcessInstance{}, nil, err
	...
	if instance.Status != domain.InstanceStatusRunning {
		return domain.ProcessInstance{}, nil, fmt.Errorf("process instance %s is not running", instance.ID)
	}
	...
	if !completed {
		...
		return domain.ProcessInstance{}, nil, fmt.Errorf("task %s status %s cannot be completed", latestTask.ID, latestTask.Status)
	}
	...
	if err := r.leaveNode(ctx, repos, def, task.InstanceID, exec.ID, task.NodeID); err != nil {
		if failErr := r.failInstance(ctx, repos, task.InstanceID, err); failErr != nil {
			return domain.ProcessInstance{}, nil, failErr
		}
		workflowErr = err
	}
	...
	return latest, workflowErr, nil
}

// FailTask 标记外部任务执行失败，并将流程实例置为失败。
func (r *PersistentRuntime) FailTask(ctx context.Context, req FailTaskRequest) (domain.ProcessInstance, error) {
	var result domain.ProcessInstance
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		task, err := repos.Tasks().Get(ctx, req.TaskID)
		if err != nil {
		...
		if task.Type != domain.TaskTypeExternal {
			return fmt.Errorf("task %s is not an external task", task.ID)
		}
		if err := verifyCallbackToken(task, req.CallbackToken); err != nil {
			return err
		}
		...
		if !taskOpen(task.Status) {
			return fmt.Errorf("task %s status %s cannot be failed", task.ID, task.Status)
		}
		if instance.Status != domain.InstanceStatusRunning {
		...
		activity.Status = domain.ActivityStatusFailed
		...
		payload["errorCode"] = strings.TrimSpace(req.ErrorCode)
		payload["message"] = message
		payload["retryable"] = req.Retryable
		payload["workerTaskId"] = strings.TrimSpace(req.WorkerTaskID)
		if err := r.auditEvent(ctx, repos, task.InstanceID, domain.AuditTaskFailed, task.NodeID, task.ID, payload); err != nil {
			return err
		}
		if err := repos.Outbox().Save(ctx, newTaskLifecycleOutbox(r.ids, r.clock.Now(), string(domain.OutboxEventTaskFailed), task)); err != nil {
			return err
		}
		if err := repos.Outbox().Save(ctx, newNotificationOutbox(r.ids, r.clock.Now(), "task_failed", task)); err != nil {
			return err
		}
		if err := r.failInstance(ctx, repos, task.InstanceID, fmt.Errorf("external task %s failed: %s", task.ID, message)); err != nil {
			return err
		}
		result, err = repos.Instances().Get(ctx, task.InstanceID)
		return err
	})
	return result, err
}
...
func (r *PersistentRuntime) RetryTask(ctx context.Context, req RetryTaskRequest) (domain.Task, error) {
	...
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		...
		if task.Status != domain.TaskStatusFailed {
			return fmt.Errorf("task %s status %s cannot be retried", task.ID, task.Status)
		}
		instance, err := repos.Instances().Get(ctx, task.InstanceID)
		if err != nil {
			return err
		}
		if instance.Status != domain.InstanceStatusFailed && instance.Status != domain.InstanceStatusRunning {
			return fmt.Errorf("process instance %s status %s cannot retry task", instance.ID, instance.Status)
		}
		def, err := repos.Definitions().FindDeployedVersion(ctx, instance.DefinitionKey, instance.DefinitionVersion)
		if err != nil {
			return err
		}
		node, ok := def.Model.Nodes[task.NodeID]
		if !ok {
			return fmt.Errorf("node %s not found", task.NodeID)
		}
		...
		task.Action = "retry"
		task.Comment = strings.TrimSpace(req.Reason)
		task.CompletedBy = strings.TrimSpace(req.Operator)
		...
		activity.Status = domain.ActivityStatusWaiting
		...
		if err := r.auditEvent(ctx, repos, task.InstanceID, domain.AuditTaskRetried, task.NodeID, task.ID, map[string]any{
			"operator": strings.TrimSpace(req.Operator),
			"reason":   strings.TrimSpace(req.Reason),
		}); err != nil {
			return err
		}
		if err := r.createDispatchOutbox(ctx, repos, task.InstanceID, task, node); err != nil {
			return err
		}
		result = task
		return nil
...
func (r *PersistentRuntime) ScanTimeoutTasks(ctx context.Context, req ScanTimeoutTasksRequest) (ScanTimeoutTasksResult, error) {
	...
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		...
		for _, task := range tasks {
			task, err = repos.Tasks().Get(ctx, task.ID)
			if err != nil {
				return err
			}
			if !taskOpen(task.Status) {
			...
			node := def.Model.Nodes[task.NodeID]
			policy := normalizeSLAPolicy(node.SLAPolicy)
			...
			if policy.Action == domain.SLAActionEscalate {
				escalationEvent := newNotificationOutbox(r.ids, r.clock.Now(), "task_timeout_escalated", task)
				if err := repos.Outbox().Save(ctx, escalationEvent); err != nil {
					return err
				}
				result.Events = append(result.Events, escalationEvent)
			}
			if err := r.applySLAPolicyInTx(ctx, repos, task, policy); err != nil {
				return err
			}
		}
		return nil
...
func (r *PersistentRuntime) applySLAPolicyInTx(ctx context.Context, repos persistence.Repositories, task domain.Task, policy domain.SLAPolicy) error {
	switch policy.Action {
	case domain.SLAActionNotify, domain.SLAActionEscalate:
		return nil
	case domain.SLAActionReject:
		_, _, err := r.completeTaskInTx(ctx, repos, CompleteTaskRequest{
			TaskID:    task.ID,
		...
		return nil
	case domain.SLAActionSuspend:
		_, err := r.transitInstanceStatusInTx(ctx, repos, InstanceLifecycleRequest{
			InstanceID: task.InstanceID,
			Operator:   policy.Operator,
			Reason:     policy.Reason,
		}, domain.InstanceStatusSuspended)
		return err
	case domain.SLAActionTerminate:
		_, err := r.terminateInstanceInTx(ctx, repos, InstanceLifecycleRequest{
			InstanceID: task.InstanceID,
			Operator:   policy.Operator,
			Reason:     policy.Reason,
		})
		return err
	default:
		return fmt.Errorf("unsupported SLA action %q", policy.Action)
	}
}

// TerminateInstance 强制终止一个运行中的流程实例（节点卡住、外部任务彻底失败、人工介入等）。
func (r *PersistentRuntime) TerminateInstance(ctx context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	var result domain.ProcessInstance
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		var err error
		result, err = r.terminateInstanceInTx(ctx, repos, req)
		return err
	})
	return result, err
}

func (r *PersistentRuntime) terminateInstanceInTx(ctx context.Context, repos persistence.Repositories, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	instance, err := repos.Instances().Get(ctx, req.InstanceID)
	if err != nil {
		return domain.ProcessInstance{}, err
	}
	if instanceClosed(instance.Status) {
		return instance, nil
	}
	now := r.clock.Now()
	instance.Status = domain.InstanceStatusTerminated
	instance.EndTime = &now
	instance.FailReason = strings.TrimSpace(req.Reason)
	instance.Version++
	if err := repos.Instances().Update(ctx, instance); err != nil {
		return domain.ProcessInstance{}, err
	}
	if err := r.cancelOpenWork(ctx, repos, instance.ID, now); err != nil {
		return domain.ProcessInstance{}, err
	}
	if err := r.setVariable(ctx, repos, instance.ID, "sys.status", string(instance.Status), domain.VariableScopeSystem, "", ""); err != nil {
		return domain.ProcessInstance{}, err
	}
	if err := r.auditEvent(ctx, repos, instance.ID, domain.AuditInstanceTerminated, "", "", map[string]any{
		"operator": strings.TrimSpace(req.Operator),
		"reason":   strings.TrimSpace(req.Reason),
	}); err != nil {
		return domain.ProcessInstance{}, err
...
// SuspendInstance 挂起一个运行中的流程实例。
func (r *PersistentRuntime) SuspendInstance(ctx context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	return r.transitInstanceStatus(ctx, req, domain.InstanceStatusSuspended)
}

// ResumeInstance 将挂起的流程实例恢复为运行状态。
func (r *PersistentRuntime) ResumeInstance(ctx context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	return r.transitInstanceStatus(ctx, req, domain.InstanceStatusRunning)
}

func (r *PersistentRuntime) transitInstanceStatus(ctx context.Context, req InstanceLifecycleRequest, target domain.InstanceStatus) (domain.ProcessInstance, error) {
	var result domain.ProcessInstance
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		var err error
		result, err = r.transitInstanceStatusInTx(ctx, repos, req, target)
		return err
	})
	return result, err
}

func (r *PersistentRuntime) transitInstanceStatusInTx(ctx context.Context, repos persistence.Repositories, req InstanceLifecycleRequest, target domain.InstanceStatus) (domain.ProcessInstance, error) {
	instance, err := repos.Instances().Get(ctx, req.InstanceID)
	if err != nil {
		return domain.ProcessInstance{}, err
	}
	...
	case domain.InstanceStatusSuspended:
		if instance.Status != domain.InstanceStatusRunning {
			return instance, fmt.Errorf("process instance %s status %s cannot be suspended", instance.ID, instance.Status)
		}
		auditType = domain.AuditInstanceSuspended
	case domain.InstanceStatusRunning:
		if instance.Status != domain.InstanceStatusSuspended {
			return instance, fmt.Errorf("process instance %s status %s cannot be resumed", instance.ID, instance.Status)
		}
		auditType = domain.AuditInstanceResumed
	default:
		return instance, fmt.Errorf("unsupported instance status transition to %s", target)
	}
	...
	if err := r.auditEvent(ctx, repos, instance.ID, auditType, "", "", map[string]any{
		"operator": strings.TrimSpace(req.Operator),
		"reason":   strings.TrimSpace(req.Reason),
	}); err != nil {
		return domain.ProcessInstance{}, err
...
// cancelOpenWork 取消实例下全部未完成的任务和执行分支（终止时使用）。
func (r *PersistentRuntime) cancelOpenWork(ctx context.Context, repos persistence.Repositories, instanceID string, now time.Time) error {
	executions, err := repos.Instances().ExecutionsByInstance(ctx, instanceID)
	if err != nil {
		return err
	}
	for _, exec := range executions {
		if exec.Status == domain.ExecutionStatusCompleted || exec.Status == domain.ExecutionStatusCanceled {
			continue
		}
		exec.Status = domain.ExecutionStatusCanceled
		exec.UpdatedAt = now
		if err := repos.Instances().SaveExecution(ctx, exec); err != nil {
			return err
		}
...
// Variables 查询流程实例当前业务变量。
func (r *PersistentRuntime) Variables(ctx context.Context, instanceID string) (map[string]domain.Variable, error) {
	return r.uow.View(ctx).Variables().CurrentByInstanceAndScope(ctx, instanceID, domain.VariableScopeBusiness)
}

// VariablesByNames 按变量名查询流程实例当前业务变量。
func (r *PersistentRuntime) VariablesByNames(ctx context.Context, instanceID string, names []string) (map[string]domain.Variable, error) {
	return r.uow.View(ctx).Variables().CurrentByNamesAndScope(ctx, instanceID, normalizeVariableNames(names), domain.VariableScopeBusiness)
}

// UpdateVariables 批量更新流程实例当前业务变量。
func (r *PersistentRuntime) UpdateVariables(ctx context.Context, req UpdateVariablesRequest) (map[string]domain.Variable, error) {
	...
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		for name, value := range req.Variables {
			...
			current, err := repos.Variables().CurrentByNames(ctx, req.InstanceID, []string{name})
			if err != nil {
				return err
			}
			if existing, ok := current[name]; ok && existing.Scope != domain.VariableScopeBusiness {
				return fmt.Errorf("system variable %s cannot be updated", name)
			}
			if err := r.setVariable(ctx, repos, req.InstanceID, name, value, domain.VariableScopeBusiness, "", ""); err != nil {
				return err
			}
	...
	return r.VariablesByNames(ctx, req.InstanceID, names)
}

// DeleteVariables 批量删除流程实例当前业务变量，返回实际删除前的变量值。
func (r *PersistentRuntime) DeleteVariables(ctx context.Context, req VariableNamesRequest) (map[string]domain.Variable, error) {
	var deleted map[string]domain.Variable
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		names := normalizeVariableNames(req.Names)
		var err error
		current, err := repos.Variables().CurrentByNames(ctx, req.InstanceID, names)
		if err != nil {
			return err
		}
		for _, name := range names {
			if existing, ok := current[name]; ok && existing.Scope != domain.VariableScopeBusiness {
				return fmt.Errorf("system variable %s cannot be deleted", name)
			}
		}
		deleted = filterVariablesByScope(current, domain.VariableScopeBusiness)
		if len(deleted) == 0 {
			return nil
		}
		now := r.clock.Now()
		for _, name := range names {
			...
			if err := r.auditEvent(ctx, repos, req.InstanceID, domain.AuditVariableDeleted, "", "", map[string]any{"name": name, "scope": old.Scope}); err != nil {
				return err
			}
		}
		return repos.Variables().DeleteCurrentByNamesAndScope(ctx, req.InstanceID, names, domain.VariableScopeBusiness)
	})
	if err != nil {
		return nil, err
	}
	if deleted == nil {
		deleted = map[string]domain.Variable{}
	}
	return deleted, nil
}

// SystemVariables 查询流程实例的系统上下文。
func (r *PersistentRuntime) SystemVariables(ctx context.Context, instanceID string) (InstanceSystemVariables, error) {
	repos := r.uow.View(ctx)
	instance, err := repos.Instances().Get(ctx, instanceID)
	if err != nil {
		return InstanceSystemVariables{}, err
	}
	def, err := repos.Definitions().Get(ctx, instance.DefinitionID)
	if err != nil {
		return InstanceSystemVariables{}, err
	}
	return instanceSystemVariables(instance, def), nil
}
...
// OutboxEvents 查询 outbox 事件（带过滤与分页）。
func (r *PersistentRuntime) OutboxEvents(ctx context.Context, filter domain.OutboxListFilter) (domain.PageResult[domain.OutboxEvent], error) {
	return r.uow.View(ctx).Outbox().List(ctx, filter)
}

// PurgeProcessedOutbox 清理已发送/死信 outbox 事件，防止表膨胀。
func (r *PersistentRuntime) PurgeProcessedOutbox(ctx context.Context, limit int) (int64, error) {
	var count int64
	err := r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		var err error
		count, err = repos.Outbox().PurgeProcessed(ctx, limit)
		return err
	})
	return count, err
}
...
func (r *PersistentRuntime) MarkOutboxDead(ctx context.Context, eventID string) error {
	return r.uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		event, err := repos.Outbox().Get(ctx, eventID)
		if err != nil {
			return err
		}
		if err := repos.Outbox().MarkDead(ctx, eventID); err != nil {
			return err
		}
		return r.failExternalDispatchFromDeadOutbox(ctx, repos, event)
	})
}

func (r *PersistentRuntime) failExternalDispatchFromDeadOutbox(ctx context.Context, repos persistence.Repositories, event domain.OutboxEvent) error {
	if event.EventType != string(domain.OutboxEventExternalTaskDispatch) {
		return nil
	}
	taskID := event.AggregateID
	if taskID == "" {
		if payloadTaskID, ok := event.Payload["taskId"].(string); ok {
	...
	if task.Status == domain.TaskStatusCompleted || task.Status == domain.TaskStatusCanceled || task.Status == domain.TaskStatusFailed {
		return nil
	}
	task.Status = domain.TaskStatusFailed
	task.Comment = fmt.Sprintf("outbox event %s reached DEAD", event.ID)
	if err := repos.Tasks().Update(ctx, task); err != nil {
		return err
	}
	return r.failInstance(ctx, repos, task.InstanceID, fmt.Errorf("external task %s dispatch failed permanently", task.ID))
}
...
func (r *PersistentRuntime) arriveGatewayJoin(ctx context.Context, repos persistence.Repositories, instanceID, executionID string, node domain.Node) (bool, error) {
	...
	executions, err := repos.Instances().ExecutionsByInstanceForUpdate(ctx, instanceID)
	if err != nil {
		return false, err
	}
...
func (r *PersistentRuntime) evaluateExclusiveGateway(ctx context.Context, repos persistence.Repositories, def domain.ProcessDefinition, instanceID, executionID string, node domain.Node) error {
	...
	for i := range outgoing {
		...
		if conditionMatches(flow.Condition, values) {
			selected = &flow
			break
		}
	}
...
func (r *PersistentRuntime) createDispatchOutbox(ctx context.Context, repos persistence.Repositories, instanceID string, task domain.Task, node domain.Node) error {
	values, err := r.inputVariableValues(ctx, repos, instanceID, node)
	if err != nil {
		return err
	}
	...
	definition, err := repos.Definitions().Get(ctx, instance.DefinitionID)
	if err != nil {
		return err
	}
	payload := domain.ExternalTaskDispatchPayload{
		TaskID:            task.ID,
		...
		InstanceName:      instance.BusinessKey,
		NodeID:            node.ID,
		NodeName:          node.Name,
		AutomationTaskKey: automationTaskKey(node),
	...
	if node.RetryPolicy.MaxAttempts > 0 {
		payload.RetryMaxAttempts = node.RetryPolicy.MaxAttempts
	}
	if node.RetryPolicy.BackoffSeconds > 0 {
		payload.RetryBackoffSeconds = node.RetryPolicy.BackoffSeconds
	}
	event, err := domain.NewOutboxEvent(r.ids.Next("outbox"), string(domain.OutboxEventExternalTaskDispatch), task.ID, payload, domain.OutboxStatusNew, r.clock.Now())
	if err != nil {
		return err
	}
...
func (r *PersistentRuntime) inputVariableValues(ctx context.Context, repos persistence.Repositories, instanceID string, node domain.Node) (map[string]any, error) {
	names, err := collectInputVariableRefs(node)
	if err != nil {
		return nil, err
	}
	if len(names) == 0 {
	...
	variables, err := repos.Variables().CurrentByNames(ctx, instanceID, names)
	if err != nil {
		return nil, err
	}
	result := map[string]any{}
