...
func NewClient(cfg Config) (*Client, error) {
	...
	// 在阿里云 ECS 内网环境下使用内网域名（如 oss-cn-hangzhou→oss-cn-hangzhou-internal）
	if cfg.UseInternalURL && !strings.Contains(endpoint, "-internal") {
		// 只处理 oss-* 开头的公网 endpoint，自定义域名保持原样
		if strings.HasPrefix(endpoint, "oss-") && strings.HasSuffix(endpoint, ".aliyuncs.com") {
			endpoint = strings.TrimSuffix(endpoint, ".aliyuncs.com") + "-internal.aliyuncs.com"
		}
	}

	ossClient, err := alioss.New(endpoint, cfg.AccessKeyID, cfg.AccessKeySecret)
	if err != nil {
...
// SignURL 生成签名 URL。expires 为有效期（time.Duration），与其它 storage 组件保持一致。
func (c *Client) SignURL(_ context.Context, key string, expires time.Duration) (string, error) {
	sec := int64(expires.Seconds())
	if sec <= 0 {
		sec = 3600
	}
	return c.bucket.SignURL(key, alioss.HTTPGet, sec)
}
