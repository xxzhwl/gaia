...
// 支持的配置项：URI、Database、ConnectTimeoutSec、MaxPoolSize、MinPoolSize
func NewClientWithSchema(schema string) (*Client, error) {
	cfg := Config{
		...
		MaxPoolSize: uint64(gaia.GetSafeConfInt64(schema + ".MaxPoolSize")),
		MinPoolSize: uint64(gaia.GetSafeConfInt64(schema + ".MinPoolSize")),
	}
