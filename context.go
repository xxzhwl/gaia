...
// traceDataSweepInterval 后台清扫周期
const traceDataSweepInterval = 10 * time.Minute

func init() {
	traceDataStack = make(map[string]*TraceData)
	go sweepTraceDataLoop()
}

// sweepTraceDataLoop 周期性清理孤儿 TraceData entry。
//
// 为什么需要它：
//   - traceDataStack 用 goroutine-id 做 key，依赖业务在 goroutine 结束时调
//     RemoveContextTrace。一旦业务自己 go func(){} 内 panic 未 recover、或
//     忘记清理，对应 entry 会永远留在 map 里，长跑服务下持续内存泄漏。
//   - 库内置入口（g.Go / grpoolgt / server handler / asynctask worker）都已
//     defer RemoveContextTrace，但无法约束业务手写的 go 语句。
//
// 判定方式：基于 goroutine 存活探测，而非固定 TTL。
//   - 周期性抓取当前进程所有存活 goroutine 的 id 集合（runtime.Stack 全栈）。
//   - traceDataStack 里"key 不在该集合中"的 entry，才判定为孤儿并删除。
//   - 这样长任务、被 CloneTraceData 传递的 context 都不会被误清——
//     只要对应 goroutine 还活着，它的 entry 就一定保留；goroutine 一旦退出，
//     其 goid 不再被复用到本进程的存活集合里（即便 runtime 内部复用 id 数字，
//     退出后到下次清扫之间也不会再出现在存活集合中）。
//
// 为什么不用固定 TTL：
//   - 固定 TTL 无法兼顾"长任务不被误清"与"泄漏尽快回收"——任何固定值都会
//     在某些场景误清仍在使用的 entry，导致日志 LogId 变空、KvData 里的
//     租户/身份信息丢失（可能引发鉴权问题）、链路断开等静默故障。
func sweepTraceDataLoop() {
	ticker := time.NewTicker(traceDataSweepInterval)
	defer ticker.Stop()
	for range ticker.C {
		live := aliveGoroutineIDs()
		rwTraceDataStack.Lock()
		orphans := 0
		for goid := range traceDataStack {
			if _, ok := live[goid]; !ok {
				delete(traceDataStack, goid)
				orphans++
			}
		}
		rwTraceDataStack.Unlock()
		if orphans > 0 {
			Println(LogWarnLevel, fmt.Sprintf("清理孤儿 TraceData entry %d 个", orphans))
		}
	}
}

// aliveGoroutineIDs 返回当前进程所有存活 goroutine 的 id 集合。
//
// 实现基于 runtime.Stack(buf, true)：true 表示抓全部 goroutine 的栈，
// 解析每行 "goroutine N [status]:" 头部取出 N。
//
// 注意：goid 是 runtime 内部序号，goroutine 退出后该数字可能被新 goroutine 复用。
// 但在单次清扫时刻，存活集合是快照——若某 goid 退出后又立即被新 goroutine 复用，
// 新 goroutine 会重新 BuildContextTrace 写自己的 entry（用相同 goid 覆盖），
// 所以"存活集合里包含该 goid"的判定对新 entry 仍然成立，不会误清。
func aliveGoroutineIDs() map[string]struct{} {
	// 初始给 1MB 缓冲；栈可能很大（数万 goroutine × 几百字节）。
	// runtime.Stack 在缓冲不足时会按 full=true 语义返回需要的大小，
	// 这里用循环扩容直到能装下全部。
	buf := make([]byte, 1<<20)
	for {
	...
	// 每个 goroutine 栈片段以 "goroutine 18 [running]:\n" 开头
...
// WithTraceData 将 Gaia TraceData 绑定到标准 context.Context 上，供新代码优先使用显式上下文传递。
func WithTraceData(ctx context.Context, data *TraceData) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	return context.WithValue(ctx, traceContextKey{}, data)
}

// TraceDataFromContext 从标准 context.Context 中获取 Gaia TraceData。
func TraceDataFromContext(ctx context.Context) (*TraceData, bool) {
	if ctx == nil {
		return nil, false
	}
	data, ok := ctx.Value(traceContextKey{}).(*TraceData)
	return data, ok
}

// CloneTraceData 复制 TraceData，避免跨 goroutine 共享可变 map/slice 导致数据竞争。
func CloneTraceData(data *TraceData) *TraceData {
	if data == nil {
		return nil
	}
	...
	cloned := *data
	if data.KvData != nil {
		cloned.KvData = make(map[string]interface{}, len(data.KvData))
		for k, v := range data.KvData {
			cloned.KvData[k] = v
		}
	}
	if data.traceSteps != nil {
		cloned.traceSteps = make(map[string][]StepDuration, len(data.traceSteps))
		for k, v := range data.traceSteps {
			copied := make([]StepDuration, len(v))
			copy(copied, v)
			cloned.traceSteps[k] = copied
		}
	}
	return &cloned
}
