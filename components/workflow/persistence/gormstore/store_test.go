...
func TestRepositoriesRoundTrip(t *testing.T) {
	...
	outbox, err := repos.Outbox().List(ctx, domain.OutboxListFilter{})
	if err != nil {
		t.Fatalf("list outbox: %v", err)
	}
	if outbox.List[0].Status != domain.OutboxStatusFailed || outbox.List[0].RetryCount != 1 || outbox.List[0].NextRetryAt == nil {
		t.Fatalf("outbox failure metadata missing: %#v", outbox.List[0])
	}
	failedPage, err := repos.Outbox().List(ctx, domain.OutboxListFilter{
		Status: domain.OutboxStatusFailed,
	})
	if err != nil {
		t.Fatalf("list failed outbox: %v", err)
	}
	if failedPage.Total != 1 || len(failedPage.List) != 1 || failedPage.List[0].ID != "outbox_1" {
		t.Fatalf("unexpected failed outbox page: %#v", failedPage)
	}
	...
	if outbox.Total != 1 || outbox.List[0].Status != domain.OutboxStatusDead || outbox.List[0].RetryCount != 2 {
		t.Fatalf("outbox dead metadata missing: %#v", outbox)
	}
	purged, err := repos.Outbox().PurgeProcessed(ctx, 1)
	if err != nil {
		t.Fatalf("purge processed outbox: %v", err)
	}
	if purged != 1 {
		t.Fatalf("expected to purge one processed outbox event, got %d", purged)
	}
	remaining, err := repos.Outbox().List(ctx, domain.OutboxListFilter{})
	if err != nil {
		t.Fatalf("list outbox after purge: %v", err)
	}
	if remaining.Total != 1 || remaining.List[0].ID != "outbox_notification_1" {
		t.Fatalf("unexpected remaining outbox after purge: %#v", remaining)
	}
...
func TestTaskListFiltersByTenantThroughInstance(t *testing.T) {
	ctx := context.Background()
	db := openTestDB(t)
	uow := NewUnitOfWork(db)
	...
	err := uow.WithinTx(ctx, func(ctx context.Context, repos persistence.Repositories) error {
		for _, fixture := range []struct {
			...
			{instanceID: "pi_tenant_a", taskID: "task_tenant_a", tenantID: "tenant_a", orderID: "ORDER_A"},
			{instanceID: "pi_tenant_b", taskID: "task_tenant_b", tenantID: "tenant_b", orderID: "ORDER_B"},
		} {
			if err := repos.Instances().Save(ctx, domain.ProcessInstance{
			...
			if err := repos.Tasks().Save(ctx, domain.Task{
				...
				ActivityInstanceID: "act_" + fixture.taskID,
				NodeID:             "manager_approve",
				Title:              fixture.orderID,
				Type:               domain.TaskTypeUser,
				Status:             domain.TaskStatusWaiting,
				CreatedAt:          now,
			}); err != nil {
	...
	result, err := uow.View(ctx).Tasks().List(ctx, domain.TaskListFilter{TenantID: "tenant_a"})
	if err != nil {
		t.Fatalf("list tenant tasks: %v", err)
	}
	if result.Total != 1 || len(result.List) != 1 {
		t.Fatalf("expected one tenant task, got total=%d list=%d", result.Total, len(result.List))
	}
	if result.List[0].ID != "task_tenant_a" {
		t.Fatalf("expected tenant_a task, got %#v", result.List[0])
	}
}
