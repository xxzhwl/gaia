...
func TestDispatcherDispatchesGRPCAutomationTask(t *testing.T) {
	...
	d := New(rt, WithAutomationRegistry(registry), WithGRPCInvoker(staticGRPCInvoker{conn: conn}), WithCallbackBaseURL("https://workflow.example.com"), WithEndpointValidator(&DefaultEndpointGuard{AllowPrivate: true}))
	count, err := d.DispatchPending(ctx, 10)
	if err != nil {
		t.Fatalf("dispatch pending: %v", err)
	}
	if count != 1 || worker.received.AutomationTaskKey != "send_contract" || worker.received.Variables["orderId"] != "ORDER_GRPC_1" || worker.received.DispatchMode != "ASYNC" || worker.received.FailCallbackURL == "" {
	...
	if tasks[1].Status != domain.TaskStatusDispatched {
		t.Fatalf("expected external task dispatched asynchronously, got %#v", tasks[1])
	}
	latest, ok := rt.GetInstance(instance.ID)
	if !ok || latest.Status != domain.InstanceStatusRunning {
		t.Fatalf("expected workflow still running before callback, got %#v", latest)
	}
	if _, err := rt.CompleteTask(ctx, engine.CompleteTaskRequest{
		TaskID:    tasks[1].ID,
		Variables: map[string]any{"done": true},
	}); err != nil {
		t.Fatalf("complete external task callback: %v", err)
	}
	latest, ok = rt.GetInstance(instance.ID)
	if !ok || latest.Status != domain.InstanceStatusCompleted {
		t.Fatalf("expected workflow completed after callback, got %#v", latest)
	}
}
...
func (i staticGRPCInvoker) DispatchTask(ctx context.Context, _ string, req automation.DispatchRequest) (automation.DispatchResult, error) {
	return workflowgrpc.NewAutomationWorkerClient(i.conn).DispatchTask(ctx, req)
}
