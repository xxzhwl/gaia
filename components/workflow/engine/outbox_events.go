...
// instanceClosed 判断流程实例是否已处于终态（不可再被生命周期操作改变）。
func instanceClosed(status domain.InstanceStatus) bool {
	return status == domain.InstanceStatusCompleted ||
		status == domain.InstanceStatusFailed ||
		status == domain.InstanceStatusTerminated
}
