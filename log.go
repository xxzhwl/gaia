...
const (
	...
	LogOutType
	LogDbType
	LogMqType        // 消息队列：Kafka / RocketMQ 等的生产/消费
	LogCacheType     // 缓存：Redis / 本地 cache 的 get/set/del 等
	LogAsyncTaskType // 异步任务：asynctask 投递/执行/重试
	LogJobType       // 定时任务：cron job 触发/执行
)
