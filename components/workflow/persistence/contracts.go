...
type DefinitionRepository interface {
	...
	SetStatus(ctx context.Context, definitionID string, status domain.DefinitionStatus) error
...
type InstanceRepository interface {
	...
	// ExecutionsByInstanceForUpdate 在事务内锁定实例下 execution 行，用于网关汇聚等并发敏感路径。
	ExecutionsByInstanceForUpdate(ctx context.Context, instanceID string) ([]domain.Execution, error)
...
type VariableRepository interface {
	...
	CurrentByInstanceAndScope(ctx context.Context, instanceID string, scope domain.VariableScope) (map[string]domain.Variable, error)
	CurrentByNames(ctx context.Context, instanceID string, names []string) (map[string]domain.Variable, error)
	CurrentByNamesAndScope(ctx context.Context, instanceID string, names []string, scope domain.VariableScope) (map[string]domain.Variable, error)
	DeleteCurrentByNames(ctx context.Context, instanceID string, names []string) error
	DeleteCurrentByNamesAndScope(ctx context.Context, instanceID string, names []string, scope domain.VariableScope) error
}

// OutboxRepository 定义 outbox 事件仓储接口。
type OutboxRepository interface {
	Save(ctx context.Context, event domain.OutboxEvent) error
	Get(ctx context.Context, eventID string) (domain.OutboxEvent, error)
	...
	// List 支持按状态/类型过滤并分页，替代原无参数全表扫描。
	List(ctx context.Context, filter domain.OutboxListFilter) (domain.PageResult[domain.OutboxEvent], error)
	// PurgeProcessed 清理已到达终态（SENT/DEAD）的 outbox 事件，防止表膨胀。
	// 返回清理的事件数量。
	PurgeProcessed(ctx context.Context, limit int) (int64, error)
}
