...
// GenerateUniqueObjectName 生成唯一对象名称（带密码学随机字符串，高并发下冲突概率极低）
func GenerateUniqueObjectName(prefix, extension string) string {
	timestamp := time.Now().Format("20060102_150405")
	var buf [8]byte
	if _, err := cryptoRand.Read(buf[:]); err != nil {
		// fallback使用纳秒混入，避免微平台上 crypto/rand 不可用时造成完全同名
		return fmt.Sprintf("%s_%s_%d.%s", prefix, timestamp, time.Now().UnixNano(), extension)
	}
