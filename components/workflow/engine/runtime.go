...
//
// CallbackToken 仅用于不可信的外部回调路径（worker 通过 callbackUrl 回调完成任务）。
// 当该字段非空时，运行时会以常量时间比对任务签发的回调令牌；为空时表示走宿主鉴权
// 中间件保护的可信内部路径（如人工待办完成），不强制校验令牌。
...
// FailTaskRequest 表示外部 worker 回调任务执行失败的请求。
//
// 与 CompleteTaskRequest 一样，CallbackToken 仅用于不可信外部回调路径。
// 失败回调不会继续推进流程，运行时会把外部任务和流程实例标记为 FAILED。
...
// RetryTaskRequest 表示人工重新投递一个失败的外部任务。
...
// VariableNamesRequest 表示按变量名读取或删除流程变量的请求。
type VariableNamesRequest struct {
	InstanceID string   `json:"instanceId"`
	Names      []string `json:"names"`
}

// UpdateVariablesRequest 表示批量更新流程变量的请求。
...
// InstanceSystemVariables 表示流程实例可暴露给上游的系统上下文。
...
// InstanceLifecycleRequest 表示对流程实例执行生命周期操作（终止/挂起/恢复）的请求。
...
func (r *Runtime) UpdateDefinition(_ context.Context, definitionID string, def domain.ProcessDefinition) (domain.ProcessDefinition, error) {
	...
	if def.EditVersion != existing.EditVersion {
		return domain.ProcessDefinition{}, fmt.Errorf("definition %s edit conflict: expected version %d got %d",
...
// SetDefinitionStatus 设置流程定义状态，用于停用（DISABLED）或重新启用（DEPLOYED）。
//
// 停用后的定义无法再用于发起新实例（findDefinition 仅匹配 DEPLOYED）；
// 已运行的实例不受影响。草稿定义不可直接停用/启用。
func (r *Runtime) SetDefinitionStatus(_ context.Context, definitionID string, status domain.DefinitionStatus) (domain.ProcessDefinition, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	def.Status = status
	r.definitions[definitionID] = def
	// 维护按 key 的最新可运行版本指针：停用时回退到次新 DEPLOYED；启用时若版本更高则指向自己。
	r.refreshLatestByKey(def.Key)
	return def, nil
}

// refreshLatestByKey 重算某个 key 的最新已部署版本指针。
func (r *Runtime) refreshLatestByKey(key string) {
	bestID := ""
	bestVersion := 0
	for id, def := range r.definitions {
		if def.Key != key || def.Status != domain.DefinitionStatusDeployed {
			continue
		}
		if def.Version > bestVersion {
			bestVersion = def.Version
			bestID = id
		}
	}
	if bestID == "" {
		delete(r.latestByKey, key)
		return
	}
	r.latestByKey[key] = bestID
}
...
func (r *Runtime) CompleteTask(_ context.Context, req CompleteTaskRequest) (domain.ProcessInstance, error) {
	...
	return r.completeTaskLocked(req, true)
}

func (r *Runtime) completeTaskLocked(req CompleteTaskRequest, verifyToken bool) (domain.ProcessInstance, error) {
...
func (r *Runtime) FailTask(_ context.Context, req FailTaskRequest) (domain.ProcessInstance, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if task.Type != domain.TaskTypeExternal {
		return domain.ProcessInstance{}, fmt.Errorf("task %s is not an external task", task.ID)
	}
	if err := verifyCallbackToken(task, req.CallbackToken); err != nil {
		return domain.ProcessInstance{}, err
	}
	...
	if !taskOpen(task.Status) {
		return domain.ProcessInstance{}, fmt.Errorf("task %s status %s cannot be failed", task.ID, task.Status)
	}
	if instance.Status != domain.InstanceStatusRunning {
		return instance, fmt.Errorf("process instance %s is not running", instance.ID)
	}
	...
	task.CompletedBy = strings.TrimSpace(req.Operator)
	r.tasks[task.ID] = task
	if activity, ok := r.activities[task.ActivityInstanceID]; ok {
	...
	r.auditEvent(task.InstanceID, domain.AuditTaskFailed, task.NodeID, task.ID, payload)
	r.createTaskLifecycleOutbox(string(domain.OutboxEventTaskFailed), task)
	r.createNotificationOutbox("task_failed", task)
	return r.failInstance(task.InstanceID, fmt.Errorf("external task %s failed: %s", task.ID, message)), nil
}

// RetryTask 重新投递一个失败的外部任务。
func (r *Runtime) RetryTask(_ context.Context, req RetryTaskRequest) (domain.Task, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if task.Type != domain.TaskTypeExternal {
		return domain.Task{}, fmt.Errorf("task %s is not an external task", task.ID)
	}
	if task.Status != domain.TaskStatusFailed {
		return domain.Task{}, fmt.Errorf("task %s status %s cannot be retried", task.ID, task.Status)
	}
	instance := r.instances[task.InstanceID]
	if instance.Status != domain.InstanceStatusFailed && instance.Status != domain.InstanceStatusRunning {
		return domain.Task{}, fmt.Errorf("process instance %s status %s cannot retry task", instance.ID, instance.Status)
	}
	def := r.definitionsByInst[task.InstanceID]
	node, ok := def.Model.Nodes[task.NodeID]
	if !ok {
		return domain.Task{}, fmt.Errorf("node %s not found", task.NodeID)
	}
	...
	if activity, ok := r.activities[task.ActivityInstanceID]; ok {
		...
		r.activities[activity.ID] = activity
		if exec, ok := r.executions[activity.ExecutionID]; ok {
			exec.Status = domain.ExecutionStatusWaiting
			exec.UpdatedAt = now
			r.executions[exec.ID] = exec
		}
	}
	r.auditEvent(task.InstanceID, domain.AuditTaskRetried, task.NodeID, task.ID, map[string]any{
		"operator": strings.TrimSpace(req.Operator),
		"reason":   strings.TrimSpace(req.Reason),
	})
	if err := r.createDispatchOutbox(task.InstanceID, task, node); err != nil {
...
func (r *Runtime) ScanTimeoutTasks(_ context.Context, req ScanTimeoutTasksRequest) (ScanTimeoutTasksResult, error) {
	...
	for _, task := range tasks {
		...
		node := r.definitionsByInst[task.InstanceID].Model.Nodes[task.NodeID]
		...
		if policy.Action == domain.SLAActionEscalate {
			escalationEvent := r.createNotificationOutbox("task_timeout_escalated", task)
			result.Events = append(result.Events, escalationEvent)
		}
		if err := r.applySLAPolicyLocked(task, policy); err != nil {
			return result, err
		}
	}
	return result, nil
...
func normalizeSLAPolicy(policy domain.SLAPolicy) domain.SLAPolicy {
	if policy.Action == "" {
		policy.Action = domain.SLAActionNotify
	}
	if policy.Operator == "" {
		policy.Operator = "workflow-sla"
	}
	if policy.Reason == "" {
		policy.Reason = "task timeout"
	}
	return policy
}

func (r *Runtime) applySLAPolicyLocked(task domain.Task, policy domain.SLAPolicy) error {
	switch policy.Action {
	case domain.SLAActionNotify, domain.SLAActionEscalate:
		return nil
	case domain.SLAActionReject:
		_, err := r.completeTaskLocked(CompleteTaskRequest{
			TaskID:    task.ID,
	...
	case domain.SLAActionSuspend:
		_, err := r.suspendInstanceLocked(InstanceLifecycleRequest{
	...
	case domain.SLAActionTerminate:
		_, err := r.terminateInstanceLocked(InstanceLifecycleRequest{
			InstanceID: task.InstanceID,
			Operator:   policy.Operator,
			Reason:     policy.Reason,
		})
...
// TerminateInstance 强制终止一个运行中的流程实例。
//
// 适用于节点卡住、外部任务彻底失败、人工介入等场景：将实例置为 TERMINATED，
// 取消其全部未完成的任务和执行分支，并写入审计。已结束的实例为幂等返回。
func (r *Runtime) TerminateInstance(_ context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.terminateInstanceLocked(req)
}

func (r *Runtime) terminateInstanceLocked(req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	instance, ok := r.instances[req.InstanceID]
	if !ok {
		return domain.ProcessInstance{}, fmt.Errorf("process instance %s not found", req.InstanceID)
	}
	...
	instance.Version++
	r.instances[instance.ID] = instance
	r.cancelOpenWork(instance.ID, now)
	r.setVariable(instance.ID, "sys.status", string(instance.Status), domain.VariableScopeSystem, "", "")
	r.auditEvent(instance.ID, domain.AuditInstanceTerminated, "", "", map[string]any{
		"operator": strings.TrimSpace(req.Operator),
		"reason":   strings.TrimSpace(req.Reason),
	})
	return instance, nil
...
// SuspendInstance 挂起一个运行中的流程实例，使其暂停推进（任务保留，可恢复）。
func (r *Runtime) SuspendInstance(_ context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.suspendInstanceLocked(req)
}

func (r *Runtime) suspendInstanceLocked(req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	instance, ok := r.instances[req.InstanceID]
	if !ok {
		return domain.ProcessInstance{}, fmt.Errorf("process instance %s not found", req.InstanceID)
	}
	...
	instance.Status = domain.InstanceStatusSuspended
	instance.Version++
	r.instances[instance.ID] = instance
	r.setVariable(instance.ID, "sys.status", string(instance.Status), domain.VariableScopeSystem, "", "")
	r.auditEvent(instance.ID, domain.AuditInstanceSuspended, "", "", map[string]any{
		"operator": strings.TrimSpace(req.Operator),
		"reason":   strings.TrimSpace(req.Reason),
	})
	return instance, nil
...
func (r *Runtime) ResumeInstance(_ context.Context, req InstanceLifecycleRequest) (domain.ProcessInstance, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	instance, ok := r.instances[req.InstanceID]
	if !ok {
	...
	r.instances[instance.ID] = instance
	r.setVariable(instance.ID, "sys.status", string(instance.Status), domain.VariableScopeSystem, "", "")
	r.auditEvent(instance.ID, domain.AuditInstanceResumed, "", "", map[string]any{
		"operator": strings.TrimSpace(req.Operator),
	})
	return instance, nil
}
...
func (r *Runtime) cancelOpenWork(instanceID string, now time.Time) {
...
func (r *Runtime) ListTasks(filter domain.TaskListFilter) domain.PageResult[domain.Task] {
	...
	for _, task := range r.tasks {
		...
		if filter.TenantID != "" {
			instance, ok := r.instances[task.InstanceID]
			if !ok || instance.TenantID != filter.TenantID {
				continue
			}
		}
...
func (r *Runtime) Variables(instanceID string) map[string]domain.Variable {
	r.mu.Lock()
	defer r.mu.Unlock()
	return filterVariablesByScope(r.variables[instanceID], domain.VariableScopeBusiness)
}

// VariablesByNames 按变量名查询流程实例当前业务变量。
func (r *Runtime) VariablesByNames(instanceID string, names []string) map[string]domain.Variable {
	r.mu.Lock()
	defer r.mu.Unlock()
	return filterVariablesByScope(filterVariableMap(r.variables[instanceID], normalizeVariableNames(names)), domain.VariableScopeBusiness)
}

// UpdateVariables 批量更新流程实例当前业务变量。
func (r *Runtime) UpdateVariables(req UpdateVariablesRequest) (map[string]domain.Variable, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	for name, value := range req.Variables {
		...
		if existing, ok := r.variables[req.InstanceID][name]; ok && existing.Scope != domain.VariableScopeBusiness {
			return nil, fmt.Errorf("system variable %s cannot be updated", name)
		}
		r.setVariable(req.InstanceID, name, value, domain.VariableScopeBusiness, "", "")
		names = append(names, name)
	}
	return filterVariableMap(r.variables[req.InstanceID], normalizeVariableNames(names)), nil
}

// DeleteVariables 批量删除流程实例当前业务变量，返回实际删除前的变量值。
func (r *Runtime) DeleteVariables(req VariableNamesRequest) (map[string]domain.Variable, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	names := normalizeVariableNames(req.Names)
	current := r.variables[req.InstanceID]
	for _, name := range names {
		if existing, ok := current[name]; ok && existing.Scope != domain.VariableScopeBusiness {
			return nil, fmt.Errorf("system variable %s cannot be deleted", name)
		}
	}
	deleted := filterVariablesByScope(filterVariableMap(current, names), domain.VariableScopeBusiness)
	if len(deleted) == 0 {
		return deleted, nil
	}
	now := r.clock.Now()
	for _, name := range names {
		...
		r.auditEvent(req.InstanceID, domain.AuditVariableDeleted, "", "", map[string]any{"name": name, "scope": old.Scope})
	}
	return deleted, nil
}

// SystemVariables 查询流程实例的系统上下文。
func (r *Runtime) SystemVariables(instanceID string) (InstanceSystemVariables, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	instance, ok := r.instances[instanceID]
	if !ok {
		return InstanceSystemVariables{}, false
	}
	def := r.definitionsByInst[instanceID]
	if def.ID == "" {
		def = r.definitions[instance.DefinitionID]
	}
	return instanceSystemVariables(instance, def), true
}
...
func (r *Runtime) OutboxEvents(filter domain.OutboxListFilter) domain.PageResult[domain.OutboxEvent] {
	r.mu.Lock()
	defer r.mu.Unlock()
	all := make([]domain.OutboxEvent, 0, len(r.outbox))
	...
	return pageSlice(all, filter.PageRequest)
}

// PurgeProcessedOutbox 删除已处于终态（SENT/DEAD）的 outbox 事件，防止内存膨胀。
func (r *Runtime) PurgeProcessedOutbox(limit int) int {
	r.mu.Lock()
	defer r.mu.Unlock()
	if limit <= 0 {
		limit = 500
	...
	purged := 0
	for id, event := range r.outbox {
		if purged >= limit {
			break
		}
		if event.Status == domain.OutboxStatusSent || event.Status == domain.OutboxStatusDead {
			delete(r.outbox, id)
			purged++
		}
	}
...
func (r *Runtime) MarkOutboxDead(eventID string) error {
	...
	event.RetryCount++
	r.outbox[eventID] = event
	r.failExternalDispatchFromDeadOutbox(event)
	return nil
}

func (r *Runtime) failExternalDispatchFromDeadOutbox(event domain.OutboxEvent) {
	...
	if !ok || task.Type != domain.TaskTypeExternal {
		return
	}
	...
	r.tasks[task.ID] = task
	r.failInstance(task.InstanceID, fmt.Errorf("external task %s dispatch failed permanently", task.ID))
}
...
func (r *Runtime) createDispatchOutbox(instanceID string, task domain.Task, node domain.Node) error {
	...
	values := r.variableValuesByNames(instanceID, names)
	payloadVars, err := buildInputPayload(node, values)
	if err != nil {
		return err
	}
	instance := r.instances[instanceID]
	definition := r.definitionsByInst[instanceID]
	if definition.ID == "" {
		definition = r.definitions[instance.DefinitionID]
	}
...
func (r *Runtime) variableValuesByNames(instanceID string, names []string) map[string]any {
	if len(names) == 0 {
		return map[string]any{}
	}
	result := map[string]any{}
	for _, name := range names {
		if variable, ok := r.variables[instanceID][name]; ok {
			result[name] = variable.Value
		}
	}
	return result
...
func filterVariableMap(input map[string]domain.Variable, names []string) map[string]domain.Variable {
	result := map[string]domain.Variable{}
	for _, name := range names {
		if variable, ok := input[name]; ok {
...
func filterVariablesByScope(input map[string]domain.Variable, scope domain.VariableScope) map[string]domain.Variable {
	result := map[string]domain.Variable{}
...
func normalizeVariableNames(names []string) []string {
...
func instanceSystemVariables(instance domain.ProcessInstance, def domain.ProcessDefinition) InstanceSystemVariables {
	return InstanceSystemVariables{
		InstanceID:        instance.ID,
