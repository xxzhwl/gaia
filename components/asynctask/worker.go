...
func (s *Scheduler) wakeUpWorker(i int32) {
	...
	if spawned > 0 {
		s.counters.workerUp.Add(int64(spawned))
		recordWorkerScale(context.Background(), s.Theme, true, spawned)
	}
}
...
func (s *Scheduler) work() {
	...
	for {
		...
		case taskId := <-s.taskIdChan:
			...
			// 计算在内存队列中的等待耗时
			if enqAt, ok := s.popEnqueueAt(taskId); ok {
				recordQueueWait(context.Background(), s.Theme, enqAt)
			}
		...
		case <-time.After(time.Second * 5):
			...
			if s.workerNeedExit() {
				...
				delete(s.workerLastRunTime, gaia.GetGoRoutineId())
				s.lastRemoveWorkerRw.Unlock()
				s.counters.workerDown.Add(1)
				recordWorkerScale(context.Background(), s.Theme, false, 1)
				return
			}
		}
	}
...
func (s *Scheduler) resetTaskToWait(taskId int64) {
	...
	if err != nil {
		...
		recordDBError(ctx, s.Theme, "reset_to_wait")
		return
	}
