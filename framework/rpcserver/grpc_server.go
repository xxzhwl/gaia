...
func newGrpcApp(schema, host, port string) *GrpcServer {
	...
	if _, err := metrics.Setup(context.Background(), gaia.GetSystemEnName()); err != nil {
		gaia.WarnF("[gRPC] 初始化指标系统失败: %s", err.Error())
	}
...
// RegisterService 注册服务描述（用于日志、健康检查与服务发现）。
...
func (s *GrpcServer) SetRegistry(registry rpcregistry.ServiceRegistry) {
	s.setRegistry(registry)
}

// Protocol 返回协议类型（固定为 "grpc"）。
...
func (s *GrpcServer) Run() {
	...
	go sendLifecycleNotify("gRPC", addr, s.schema, "启动", map[string]any{
	...
	<-s.stopChan
	// 同步发送停止通知，确保进程退出前发出去
	sendLifecycleNotify("gRPC", addr, s.schema, "停止", map[string]any{
		"service_count": len(s.services),
	})
}

// Stop 优雅关闭 gRPC 服务器（幂等）。
...
// sendLifecycleNotify 异步发送服务生命周期通知
