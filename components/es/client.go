...
func (c *Client) ExistsIndex(index string) bool {
	exists, err := c.ExistsIndexWithCtx(context.Background(), index)
	if err != nil {
...
func (c *Client) ExistsIndexWithCtx(ctx context.Context, index string) (bool, error) {
...
func (c *Client) CreateIndex(index string) error {
	return c.CreateIndexWithCtx(context.Background(), index)
}

func (c *Client) CreateIndexWithCtx(ctx context.Context, index string) error {
	resp, err := c.client.Indices.Create(index).Do(ctx)
	if err != nil {
		return err
	}
...
func (c *Client) CreateDoc(index string, doc any) (string, error) {
	return c.CreateDocWithCtx(context.Background(), index, doc)
}

func (c *Client) CreateDocWithCtx(ctx context.Context, index string, doc any) (string, error) {
	resp, err := c.client.Index(index).Document(doc).Do(ctx)
	if err != nil {
		return "", err
	}
	if resp == nil {
...
func (c *Client) Search(index string, query map[string]interface{}) (SearchResult, error) {
	return c.SearchWithCtx(context.Background(), index, query)
}

// SearchWithCtx 带 context 的查询
func (c *Client) SearchWithCtx(ctx context.Context, index string, query map[string]interface{}) (SearchResult, error) {
...
func getCondMap(args []OpArg) []map[string]any {
	...
	for _, opArg := range args {
		...
		default:
			// 未知操作符默认走 term 查询，同时记录警告以便业务排查参数错误
			gaia.WarnF("未知 ES 查询操作符 [%s]，默认退化为 term", opArg.Op)
			res = append(res, map[string]any{"term": map[string]any{opArg.Key: opArg.Value}})
		}
	}
	return res
