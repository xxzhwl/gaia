...
const (
	...
	EcTokenExpired int64 = 20002 //Token过期
...
type JudgeMajorErrFunc func(err error) bool

// JudgeMajorErr 判断是否是严重错误
var JudgeMajorErr func(err error) bool = func(err error) bool {
	var v LogicError
	if errors.As(err, &v) {
		code := GetCode(v)
		if code >= EcMajorMinErr && code <= EcMajorMaxErr {
			return true
		}
	}
	return false
...
// SetMajorErrJudgeFunc 设置为判断严重错误
func SetMajorErrJudgeFunc(fun JudgeMajorErrFunc) {
	JudgeMajorErr = fun
}
